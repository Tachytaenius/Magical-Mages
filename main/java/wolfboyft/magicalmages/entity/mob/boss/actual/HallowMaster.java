package wolfboyft.magicalmages.entity.mob.boss.actual;
...
public class HallowMaster extends EntityMob implements IBossDisplayData,
		IRangedAttackMob {
	...
	public HallowMaster(World worldIn) {
		super(worldIn);
	...
	public void onLivingUpdate() {
		...
		if (!this.worldObj.isRemote && this.getWatchedTargetId(0) > 0) {
			Entity entity = this.worldObj.getEntityByID(this
					.getWatchedTargetId(0));

			if (entity != null) {
				if (this.posY < entity.posY || !this.isArmored()
						&& this.posY < entity.posY + 5.0D) {
		...
		for (i = 0; i < 2; ++i) {
			...
			if (entity1 != null) {
				...
				this.field_82220_d[i] = this.func_82204_b(
						this.field_82220_d[i], f1, 40.0F);
				this.field_82221_e[i] = this.func_82204_b(
						this.field_82221_e[i], f, 10.0F);
			} else {
				this.field_82221_e[i] = this.func_82204_b(
						this.field_82221_e[i], this.renderYawOffset, 10.0F);
			}
		}
		...
		for (j = 0; j < 3; ++j) {
			...
			this.worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d10
					+ this.rand.nextGaussian() * 0.30000001192092896D, d2
					+ this.rand.nextGaussian() * 0.30000001192092896D, d4
					+ this.rand.nextGaussian() * 0.30000001192092896D, 0.0D,
					0.0D, 0.0D, new int[0]);

			if (flag && this.worldObj.rand.nextInt(4) == 0) {
				this.worldObj.spawnParticle(EnumParticleTypes.SPELL_MOB, d10
						+ this.rand.nextGaussian() * 0.30000001192092896D, d2
						+ this.rand.nextGaussian() * 0.30000001192092896D, d4
						+ this.rand.nextGaussian() * 0.30000001192092896D,
		...
		if (this.getInvulTime() > 0) {
			...
			for (i = 1; i < 3; ++i) {
				if (this.ticksExisted >= this.field_82223_h[i - 1]) {
					this.field_82223_h[i - 1] = this.ticksExisted + 10
					...
					if (this.worldObj.getDifficulty() == EnumDifficulty.NORMAL
						...
						if (l2 > 15) {
							float f = 10.0F;
							float f1 = 5.0F;
							double d0 = MathHelper.getRandomDoubleInRange(
									this.rand, this.posX - (double) f,
									this.posX + (double) f);
							double d1 = MathHelper.getRandomDoubleInRange(
									this.rand, this.posY - (double) f1,
									this.posY + (double) f1);
							double d2 = MathHelper.getRandomDoubleInRange(
									this.rand, this.posZ - (double) f,
									this.posZ + (double) f);
							this.launchWitherSkullToCoords(i + 1, d0, d1, d2,
									true);
					...
					if (i1 > 0) {
						...
						if (entity != null && entity.isEntityAlive()
							...
							this.launchWitherSkullToEntity(i + 1,
									(EntityLivingBase) entity);
							this.field_82223_h[i - 1] = this.ticksExisted + 40
									+ this.rand.nextInt(20);
							this.field_82224_i[i - 1] = 0;
						} else {
						...
						List list = this.worldObj.getEntitiesWithinAABB(
								...
								this.getEntityBoundingBox().expand(20.0D, 8.0D,
										20.0D), Predicates.and(
						...
						for (int i2 = -1; i2 <= 1; ++i2) {
							for (int j = 0; j <= 3; ++j) {
								...
								Block block = this.worldObj.getBlockState(
										new BlockPos(j2, k, l)).getBlock();

								if (!block.isAir(worldObj, new BlockPos(j2, k,
										l))
										&& block.canEntityDestroy(worldObj,
												new BlockPos(j2, k, l), this)) {
									flag = this.worldObj.destroyBlock(
											new BlockPos(j2, k, l), true)
											|| flag;
								}
							}
						}
	...
	private double func_82214_u(int p_82214_1_) {
		if (p_82214_1_ <= 0) {
			...
			float f = (this.renderYawOffset + (float) (180 * (p_82214_1_ - 1)))
	...
	private double func_82213_w(int p_82213_1_) {
		if (p_82213_1_ <= 0) {
			...
			float f = (this.renderYawOffset + (float) (180 * (p_82213_1_ - 1)))
					/ 180.0F * (float) Math.PI;
	...
	private void launchWitherSkullToEntity(int p_82216_1_,
		...
		this.launchWitherSkullToCoords(p_82216_1_, p_82216_2_.posX,
				...
				p_82216_2_.posZ, p_82216_1_ == 0
						&& this.rand.nextFloat() < 0.001F);
	}

	private void launchWitherSkullToCoords(int p_82209_1_, double p_82209_2_,
		...
		EntityWitherSkull entitywitherskull = new EntityWitherSkull(
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		if (this.isEntityInvulnerable(source)) {
			return false;
				...
				&& !(source.getEntity() instanceof HallowMaster)) {
			if (this.getInvulTime() > 0 && source != DamageSource.outOfWorld) {
				...
				if (entity != null
						...
						&& ((EntityLivingBase) entity).getCreatureAttribute() == this
	...
	public void func_82211_c(int p_82211_1_, int p_82211_2_) {
		this.dataWatcher.updateObject(17 + p_82211_1_,
				Integer.valueOf(p_82211_2_));
	}
