...
public class FarlanderPearl extends EntityThrowable {
	private static final String __OBFID = "CL_00001725";

	public FarlanderPearl(World worldIn, EntityLivingBase p_i1783_2_) {
		super(worldIn, p_i1783_2_);
	}

	@SideOnly(Side.CLIENT)
	public FarlanderPearl(World worldIn, double p_i1784_2_, double p_i1784_4_,
			double p_i1784_6_) {
	...
	protected void onImpact(MovingObjectPosition p_70184_1_) {
		...
		if (p_70184_1_.entityHit != null) {
			p_70184_1_.entityHit.attackEntityFrom(
					DamageSource.causeThrownDamage(this, entitylivingbase),
					0.0F);
		}

		for (int i = 0; i < 32; ++i) {
			this.worldObj.spawnParticle(EnumParticleTypes.PORTAL, this.posX,
					this.posY + this.rand.nextDouble() * 2.0D, this.posZ,
		...
		if (!this.worldObj.isRemote) {
			if (entitylivingbase instanceof EntityPlayerMP) {
				...
				if (entityplayermp.playerNetServerHandler.getNetworkManager()
						.isChannelOpen()
					...
					net.minecraftforge.event.entity.living.EnderTeleportEvent event = new net.minecraftforge.event.entity.living.EnderTeleportEvent(
							entityplayermp, this.posX, this.posY, this.posZ,
							5.0F);
					if (!net.minecraftforge.common.MinecraftForge.EVENT_BUS
							.post(event)) { // Don't indent to lower patch size
						...
						entitylivingbase.setPositionAndUpdate(event.targetX,
								event.targetY, event.targetZ);
						entitylivingbase.fallDistance = 0.0F;
						entitylivingbase.attackEntityFrom(DamageSource.fall,
								event.attackDamage);
					}
				}
			}
